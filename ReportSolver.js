...
$(function () {
  ...
  RS.doEdit = function (sectionNumber, comment, editSummary, status) {
    ...
    new mw.Api().postWithEditToken({
      ...
      const $editSectionLink = $('span.mw-editsection a[href$="&section=' + sectionNumber + '"]');
      const $mwEditSection = $editSectionLink.closest('.mw-editsection');
      let $headline = $mwEditSection.prevAll('.mw-headline').first();

      if ($headline.length === 0) {
        $headline = $mwEditSection.prevAll('h1, h2, h3, h4, h5, h6').first();
      }

      const headingLevel = $headline.prop('tagName').replace('H', '');
      const headingText = $headline.text();
      const heading = `\n${'='.repeat(headingLevel)} ${headingText} ${'='.repeat(headingLevel)}\n`;

      if (wgPageName === 'Steward_requests/Global_permissions') {
        comment = comment.trim();
        if (!/[.!?}]$/.test(comment)) comment += '.';
        comment += ' ~~~~';
		const updatedWikitext = fullWikitext.replace(/\|\s*status\s*=\s*[^|}]*([\|}])/i, `|status = ${status}\n $1`);
